...
description: "Best practices and prompt engineering techniques for Cursor Agent."
source: "https://cursor.com/docs/agent/prompting"
category: "Agent"
lastFetched: "2026-09-05T16:28:21.038Z"
---
...
Type `/` in the chat input to invoke a [skill](https://cursor.com/docs/skills). Pressing \`Enter\` attaches the skill to one message, and it fades as the conversation moves on. Use any skill as a Custom Mode to keep the agent focused on it while it works.

Pick the skill from the `/` menu and press \`Option+Enter\` (\`Alt+Enter\`) (Mac) or \`Alt+Enter\` (Windows) instead. You can also select **Use as Mode** from the skill entry. Inside a mode, the skill stays in context on every turn, even as the agent works for hours, until you exit the mode.
...
Custom Modes are available in the [Agents Window](./agents-window.md) and the [CLI](https://cursor.com/docs/cli/overview). Any skill with a valid frontmatter block can back a mode, and the optional `icon` and `color` frontmatter fields style the mode's badge. See [Using a skill as a Custom Mode](https://cursor.com/docs/skills#using-a-skill-as-a-custom-mode).
...
-   **Paste from clipboard** with \`Cmd+V\` (\`Ctrl+V\`), including screenshots
