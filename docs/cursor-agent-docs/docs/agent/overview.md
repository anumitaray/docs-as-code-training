...
description: "Overview of Cursor Agent: how it works, tools, checkpoints, queued messages, and goals."
source: "https://cursor.com/docs/agent/overview"
category: "Agent"
lastFetched: "2026-09-05T16:28:20.553Z"
---
...
Control a browser to take screenshots, test applications, and verify visual changes. Agent can navigate pages, interact with elements, and capture the current state for analysis. See the [Browser documentation](./tools/browser.md) for details.
...
> **⚠️ Warning:** Checkpoints are stored locally and separate from Git. Only use them for undoing Agent changes; use Git for permanent version control.

## Queued messages

You have two ways to talk to an agent while it works. Queue a message and it waits for the current task to finish. [Send a follow-up now](#steer-a-running-agent) and it steers the active turn at the agent's next tool call.

> 🎥 **Video Demonstration:** [agent-queue.mp4](https://cursor.com/docs-static/images/agent/planning/agent-queue.mp4)
...
This is available on [cursor.com/agents](https://cursor.com/agents) now and rolling out in the [Agents Window](./agents-window.md). Press \`Tab\` to queue the message for after the turn instead.

In the [CLI](https://cursor.com/docs/cli/overview), pressing \`Enter\` while the agent works steers the active run at a safe boundary, and pressing \`Enter\` again interrupts the turn.
...
In the [CLI](https://cursor.com/docs/cli/overview), \`Ctrl+C\` pauses the goal. Pair a goal with a [Custom Mode](./prompting#custom-modes.md) when you want the agent to follow a playbook, or with the built-in [`/loop`](https://cursor.com/docs/skills#built-in-cursor-skills) skill for recurring check-ins while it pursues the objective.

> **ℹ️ Note:** /goal is rolling out. If you don't see it, try it in a new chat.
